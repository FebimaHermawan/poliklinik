		...
		if ($nama != "") {
			if ($jumlah != "") {
				if ($dosis != "") {
					if ($getedob) {
						$total = $harga * $jumlah; 
						$param = array(":ido"=>$getidob, ":edo"=>$getedob ,":id"=>$getid, ":jum"=>$jumlah, ":dos"=>$dosis, ":total"=>$total);
						$db->execute("update resep set id_obat = :edo, jumlah = :jum, dosis = :dos, total = :total where id_obat = :ido and id = :id", $param);
						$db->redirect("index.php?act=cek&id=$getid&chg=$getCh");
					}
					else{
						$total = $harga * $jumlah;
						$param = array(":ido"=>$getidob, ":id"=>$getid, ":jum"=>$jumlah, ":dos"=>$dosis, ":total"=>$total);
						$db->execute("update resep set jumlah = :jum, dosis = :dos, total = :total where id_obat = :ido and id = :id", $param);
						$db->redirect("index.php?act=cek&id=$getid&chg=$getCh");	
					}
				}
				else{
