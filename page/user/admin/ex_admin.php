                  ...
                  <div class="box box-primary">
                        ...
                        <form method="post">
                              ...
                              <div class="box-footer">
                                    <input type="submit" name="btn-tambah-admin" class="btn btn-primary btn-flat" value="Tambah">
...
<?php 
	if (isset($POST["btn-tambah-admin"])) {
		        extract($POST);
            if ($nama != "") {
                  if ($username != ""){
                        if ($password != "") {
                              ...
                              if ($getUser->rowCount() > 0) {
                                  $ValUser = $getUser->fetch();
                                    if ($ValUser["username"] != $username) {
                                         ...
                                         $a = $db->execute("insert into user values(:id, :name, :username, :password, :telepon, :alamat, '0')", $param);
                                          $db->redirect("?hal=user/home&act=admin&name=Admin");

                                    }
                                    else{
                                          $db->validationInput("?hal=user/home&act=add&name=Admin&type=admin", "Username");
                                    }
                              }
                              else{
                                         $id = "Us".time()."eR".date("Y");
                                          $pass = md5($password);
                                          $param = array(":id"=>$id, ":name"=>$nama, ":username"=>$username, ":password"=>$pass, ":telepon"=>$telepon, ":alamat"=>$alamat);
                                          $db->execute("insert into user values(:id, :name, :username, :password, :telepon, :alamat, '0')", $param);
                                          $db->redirect("?hal=user/home&act=admin&name=Admin");
                              }
                        }
                        else{
                              $db->errorInput("?hal=user/home&act=add&name=Admin&type=admin", "Password");
                        }
                  }
                  else{
                        $db->errorInput("?hal=user/home&act=add&name=Admin&type=admin", "Username");
                  }
            }
            else{
                  $db->errorInput("?hal=user/home&act=add&name=Admin&type=admin", "nama");
            }
	}
