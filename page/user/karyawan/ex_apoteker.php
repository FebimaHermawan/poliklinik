                  ...
                  <div class="box box-primary">
                        ...
                        <form method="post">
                              ...
                              <div class="box-footer">
                                    <input type="submit" name="btn-tambah-apoteker" class="btn btn-primary btn-flat" value="Tambah">
...
<?php
      if (isset($POST["btn-tambah-apoteker"])) {
            extract($POST);
            if ($nama != "") {
                  if ($username != ""){
                        if ($password != "") {
                              ...
                              if ($getUser->rowCount() > 0) {
                                  while ($ValUser = $getUser->fetch()) {
                                    if ($ValUser["username"] != $username) {
                                          ...
                                          $db->execute("insert into user values(:id, :name, :username, :password, :telepon, :alamat, '2')", $param);
                              ...
                              $db->errorInput("?hal=user/home&act=add&name=Apoteker&type=apoteker", "Password");
                        }
                  }
                  else{
                        $db->errorInput("?hal=user/home&act=add&name=Apoteker&type=apoteker", "Username");
                  }
            }
            else{
                  $db->errorInput("?hal=user/home&act=add&name=Apoteker&type=apoteker", "nama");
            }
      }
