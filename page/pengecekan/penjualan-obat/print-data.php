						...
						<table class="table-print">
							...
							<?php
								...
								if ($getsd && $getfd) {
									...
									$getPob = $db->execute("select * from activity_obat where tanggal between :sd and :fd and status = 2", $param);
								}
								if (!$getsd && !$getfd) {
									$getPob = $db->getResult("select * from activity_obat where status = 2");
								}
