						...
						<table id="data-table" class="table table-stripped table-bordered">
							...
							<tbody>
								<?php
									...
									$no++;
									if ($givPen["status_pendaftaran"] == 2) {
								?>
								<tr>
									<td><?=$no?></td>
									<td>
									...
									<td>
										<?php
											$getRes = $db->execute("select id from resep where id_pendaftaran = :idp and date_format(tanggal, '%Y-%m-%d') = date_format(now(), '%Y-%m-%d')", array(":idp"=>$givPen["no_daftar"]));
											$givRes = $getRes->fetch();
										?>
										<a href="index.php?act=cek&id=<?=$givRes['id']?>&chg=<?=$givPen['no_daftar']?>">
